...
/**
 * Creates an instance of the Ltc Wallet Service.
 * @constructor
 */
...
/**
 * Gets the current version of LTCWS
 */
WalletService.getServiceVersion = function() {
  if (!serviceVersion)
    serviceVersion = 'ltcws-' + require('../package').version;
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new ltcLib.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype._clientSupportsPayProRefund = function() {
  ...
  if (version.agent != 'ltcwc') return true;
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  try {
    ltcLib.HDPublicKey(opts.xPubKey);
  } catch (ex) {
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.includes(['ltc', 'bit'], value.toLowerCase());
    },
  }];
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = ltcLib.Address(addresses[0]).toObject().network;
  ...
  async.series([
    ...
    function(next) {
      ...
      _.each(allUtxos, function(utxo) {
          if (!addressToPath[utxo.address] || !addressToPath[utxo.address].path) {
              console.error('Error in addressToPath[utxo.address].path', addressToPath[utxo.address].path, addressToPath[utxo.address], JSON.stringify(addressToPath, null, 2));
          }
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var ltcTx = txp.getLtcTx();
    ltcError = ltcTx.getSerializationError(serializationOpts);
    if (!ltcError) {
      txp.fee = ltcTx.getFee();
    }
  } catch (ex) {
    log.error('Error building Ltc transaction', ex);
    return ex;
  }

  if (ltcError instanceof ltcLib.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (ltcError instanceof ltcLib.errors.Transaction.DustOutputs)
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      log.debug('Total value in all utxos (' + Utils.formatAmountInLtc(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInLtc(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      log.debug('Value after fees in all utxos (' + Utils.formatAmountInLtc(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInLtc(txpAmount) + ')');
    ...
    log.debug('Big input threshold ' + Utils.formatAmountInLtc(bigInputThreshold));
    ...
    _.each(smallInputs, function(input, i) {
      ...
      log.debug('The input contributes ' + Utils.formatAmountInLtc(netInputAmount));
      ...
      log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInLtc(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInLtc((selected.length - 1) * feePerInput));
      ...
      log.debug('Cumuled total so far: ' + Utils.formatAmountInLtc(total) + ', Net total so far: ' + Utils.formatAmountInLtc(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        log.debug('Tx change: ', Utils.formatAmountInLtc(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, ltcLib.Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          log.debug('Change below dust threshold (' + Utils.formatAmountInLtc(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      log.debug('Could not reach Txp total (' + Utils.formatAmountInLtc(txpAmount) + '), still missing: ' + Utils.formatAmountInLtc(txpAmount - netTotal));
  ...
  log.debug('Selecting inputs for a ' + Utils.formatAmountInLtc(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet(null, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        log.debug('Fee for this selection: ' + Utils.formatAmountInLtc(fee));
      ...
      if (!err) {
        ...
        log.debug('Successfully built transaction. Total fees: ' + Utils.formatAmountInLtc(txp.fee) + ', total change: ' + Utils.formatAmountInLtc(change));
      } else {
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    var toAddress = {};
    try {
      toAddress = new ltcLib.Address(output.toAddress);
    } catch (ex) {
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.copayerId, opts.signatures, copayer.xPubKey)) {
          ...
          var raw = txp.getLtcTx().uncheckedSerialize();
          log.warn('Raw tx:', raw);
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getNormalizedTxs(addresses, from, to, cb) {
    ...
    var network = ltcLib.Address(addresses[0].address).toObject().network;
