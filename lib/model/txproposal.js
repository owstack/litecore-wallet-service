...
TxProposal.create = function(opts) {
  ...
  try {
    x.network = opts.network || ltcLib.Address(x.outputs[0].toAddress).toObject().network;
  } catch (ex) {}
...
TxProposal.prototype._buildTx = function() {
  ...
  // Validate actual inputs vs outputs independently of Ltc
...
TxProposal.prototype.getLtcTx = function() {
  var self = this;
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToLtcTx(t, x.signatures, x.xpub);
  });
...
TxProposal.prototype._addSignaturesToLtcTx = function(tx, signatures, xpub) {
  var self = this;
...
TxProposal.prototype.sign = function(copayerId, signatures, xpub) {
  try {
    ...
    var tx = this.getLtcTx();
    this._addSignaturesToLtcTx(tx, signatures, xpub);
